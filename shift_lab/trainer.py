...
class MultitaskTrainer(Trainer):
    ...
    def training_step(
        ...
        if self.args.n_gpu > 1:
            loss = {name: l.mean() for name, l in loss.items()}  # mean() to average on multi-gpu parallel training
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if loss is not None:
                losses = {
                    task_name: self.accelerator.gather_for_metrics((l.repeat(batch_size)))
                    for task_name, l in loss.items()
                }
            ...
            if self.compute_metrics_interval == "batch":
                ...
                all_losses = (
                    losses if all_losses is None
                    else {
                        task_name: np.concatenate((all_losses[task_name], losses[task_name]), axis=0)
                        for task_name in self.training_tasks
                    }
                )
        ...
        if all_losses is not None:
            combined_loss = 0.0
            for task_name in all_losses.keys():
                task_loss = all_losses[task_name].mean().item()
                metrics[f"{metric_key_prefix}_{task_name}_loss"] = task_loss
                combined_loss += round(task_loss * self.loss_lambdas[task_name].cpu().numpy(), 4)
            metrics[f"{metric_key_prefix}_loss"] = metrics.get(f"{metric_key_prefix}_loss", combined_loss)
    ...
    def _inner_training_loop(
        ...
        tr_loss = {train_task: torch.tensor(0.0).to(args.device) for train_task in self.training_tasks}
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                for train_task in tr_loss.keys():
                    if (
                        ...
                        and (torch.isnan(tr_loss_step[train_task]) or torch.isinf(tr_loss_step[train_task]))
                    ):
                        # if loss is nan or inf simply add the average of previous logged losses
                        tr_loss[train_task] += tr_loss[train_task] / (1 + self.state.global_step - self._globalstep_last_logged)
                    else:
                        tr_loss[train_task] += tr_loss_step[train_task]
        ...
        self._total_loss_scalar += sum(
            [tr_loss[train_task].item() * self.loss_lambdas[train_task].cpu().numpy() for train_task in self.training_tasks]
        )
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval, outputs=None):
        if self.control.should_log:
            ...
            for train_task in tr_loss.keys():
                tr_loss[train_task] -= tr_loss[train_task]

            logs["loss"] = {"train_loss": 0.0}
            for train_task, trl in tr_loss_scalar.items():
                task_loss = round(trl / (self.state.global_step - self._globalstep_last_logged), 4)
